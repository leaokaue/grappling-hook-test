...
func load_secret_zone():
	ResourceLoader.load_threaded_request(secret_zone_scene_path)
	#var p := func():
		#print(ResourceLoader.load_threaded_get_status(secret_zone_scene_path))
	#container_finished.connect(p)
	#label_finished.connect(p)

#what to do
#load zone in the background
#handle text
#text over, go into secret zone
...
func handle_container(container : VBoxContainer, delay : float = 1.0,line_delay : float = 0.5):
	...
	var leftover_containers : Array[VBoxContainer ]= label_containers.duplicate()
